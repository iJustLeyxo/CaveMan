    ...
    public void run() {
        ...
        if (!selected.isEmpty()) {
            Console.logL(Type.REQUESTED, Style.UNINSTALL, selected.size() +
                    " plugins(s) to unlink", 4, 21, selected.toArray());
        } else {
        ...
        for (Plugin p : selected) {
            ...
            if (!Files.isSymbolicLink(file.toPath())) {
                if (!Console.log(Type.INFO, Style.WARN, " skipped (not a symbolic link, use " +
                        Command.UNINSTALL.refs[0] + " to remove)\n")) {
                    Console.log(Type.WARN, "Uninstalling " + p.ref + " skipped (not a symbolic link, use " +
                            Command.UNINSTALL.refs[0] + " to remove)\n");
                }
                continue;
            }
